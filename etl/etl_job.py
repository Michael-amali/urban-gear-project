...
def extract_raw_data(spark: SparkSession, process_date: str) -> DataFrame:
    if PipelineConfig.ENV == "local":
        ...
        df = spark.read.schema(RAW_ORDER_SCHEMA).option("mode", "PERMISSIVE").option("columnNameOfCorruptRecord", "_corrupt_record").json(raw_path)
    else:
        raw_path = f"{PipelineConfig.S3_RAW}/orders/dt={process_date}/"
        df = spark.read.schema(RAW_ORDER_SCHEMA).option("mode", "PERMISSIVE").option("columnNameOfCorruptRecord", "_corrupt_record").json(raw_path)
    ...
    if count < PipelineConfig.MIN_RECORD_COUNT:
        raise ValueError(f"Count {count} below threshold {PipelineConfig.MIN_RECORD_COUNT}")
    return df

def tag_and_route(df: DataFrame, process_date: str):
    tagged = (
        df.withColumn("_null_critical", F.col("order_id").isNull() | F.col("customer_id").isNull() | F.col("order_date").isNull() | F.col("quantity").isNull() | F.col("unit_price").isNull())
        ...
        .withColumn("_is_rejected", F.col("_null_critical") | F.col("_negative_qty") | F.col("_future_date") | F.col("_corrupt_json"))
        .withColumn("rejection_reason", F.when(F.col("_corrupt_json"), F.lit("corrupt_json")).when(F.col("_null_critical"), F.lit("null_critical_field")).when(F.col("_negative_qty"), F.lit("negative_quantity")).when(F.col("_future_date"), F.lit("future_order_date")).otherwise(F.lit(None)))
    )
...
def transform_clean(df: DataFrame, process_date: str) -> DataFrame:
    ...
    return (
        df.withColumn("gross_revenue", F.round(F.col("quantity") * F.col("unit_price"), 2))
        .withColumn("discount_amount", F.round(F.col("gross_revenue") * F.coalesce(F.col("discount_pct"), F.lit(0.0)) / 100, 2))
        .withColumn("net_revenue", F.round(F.col("gross_revenue") - F.col("discount_amount"), 2))
        .withColumn("total_amount", F.round(F.col("net_revenue") + F.coalesce(F.col("shipping_cost"), F.lit(0.0)) + F.coalesce(F.col("tax_amount"), F.lit(0.0)), 2))
        ...
        .withColumn("order_item_count", F.count("*").over(w))
        .withColumn("order_total_revenue", F.sum("net_revenue").over(w))
        .withColumn("is_high_value", F.col("order_total_revenue") > PipelineConfig.MAX_ORDER_VALUE)
        .withColumn("etl_processed_at", F.current_timestamp())
        .withColumn("etl_process_date", F.lit(process_date))
        .drop("_null_critical", "_negative_qty", "_future_date", "_invalid_platform", "_corrupt_json", "_is_rejected", "_corrupt_record")
    )

def collect_metrics(raw_df, clean_df, rejected_df, process_date):
    total = raw_df.count()
    ...
    try:
        ...
        rule_counts = {}
    metrics = {"process_date": process_date, "total_count": total, "clean_count": clean, "rejected_count": rejected, "reject_rate": round(rejected/total, 4) if total else 0, "rule_counts": rule_counts, "generated_at": datetime.now().isoformat()}
    logger.info(f"[METRICS] {metrics}")
    return metrics

def write_outputs(clean_df, rejected_df, metrics, process_date):
    ...
    if PipelineConfig.ENV == "local":
        (clean_df.repartition(F.col("order_year"), F.col("order_month"), F.col("order_day"))
         .write.mode("overwrite").partitionBy("order_year", "order_month", "order_day").parquet(processed_path))
        if rejected_df.count() > 0:
            (rejected_df.coalesce(1).write.mode("overwrite").parquet(rejected_path))
        (metrics_dir / "metrics.json").write_text(json.dumps(metrics, indent=2))
    else:
        import boto3
        clean_df.repartition(F.col("order_year"), F.col("order_month"), F.col("order_day")).write.mode("overwrite").partitionBy("order_year", "order_month", "order_day").parquet(processed_path)
        if rejected_df.count() > 0:
            rejected_df.coalesce(1).write.mode("overwrite").parquet(rejected_path)
        s3 = boto3.client("s3", region_name=PipelineConfig.AWS_REGION)
        bucket, key = f"{PipelineConfig.S3_METRICS}/orders/dt={process_date}/metrics.json".replace("s3://", "").split("/", 1)
...
def main(process_date: str = None, env: str = None):
    ...
    logger.info(f"ETL Start date={process_date} env={PipelineConfig.ENV}")
    ...
    try:
        ...
        logger.info(f"ETL COMPLETE Clean:{metrics['clean_count']} Rejected:{metrics['rejected_count']} Rate:{metrics['reject_rate']:.2%}")
    finally:
        spark.stop()
